/*
 * http_input.c - HTTP Input Monitor for CariTranscoder
 *
 * Receives MPEG-TS streams via HTTP and outputs to UDP/multicast.
 * Provides REST API for bitrate monitoring.
 *
 * Based on hls_input.c
 *
...
typedef struct {
    // HTTP input settings
    char http_url[1024];        // HTTP URL for MPEG-TS stream
...
void print_help(const char *prog) {
    ...
    printf("HTTP Input Options:\n");
    printf("  --url URL                    HTTP URL for MPEG-TS stream (required)\n");
    printf("\nOutput Options:\n");
    ...
    printf("\nGeneral Options:\n");
    printf("  --log-file FILE              Log file path (default: /tmp/http_input.log)\n");
    ...
    printf("\nExamples:\n");
    printf("  HTTP MPEG-TS stream:\n");
    printf("    %s --url http://server:8080/stream/video.ts \\\n", prog);
...
void init_context() {
    ...
    strcpy(g_ctx.log_file, "/tmp/http_input.log");
    g_ctx.api_port = 8080;
...
static enum MHD_Result api_handler(void *cls, struct MHD_Connection *connection,
    ...
    if (strcmp(url, "/metrics") == 0) {
        ...
        char response[16384] = "{\"status\":\"running\",\"type\":\"http\",\"pids\":{";
    ...
    if (strcmp(url, "/metrics/history") == 0) {
        ...
        strcpy(response, "{\"status\":\"running\",\"type\":\"http\",\"pids\":{");
    ...
    if (strcmp(url, "/health") == 0) {
        const char *response = "{\"status\":\"ok\",\"type\":\"http\"}";
        struct MHD_Response *mhd_response = MHD_create_response_from_buffer(
            strlen(response), (void *)response, MHD_RESPMEM_MUST_COPY);
        MHD_add_response_header(mhd_response, "Content-Type", "application/json");
        MHD_add_response_header(mhd_response, "Access-Control-Allow-Origin", "*");
    ...
    if (strcmp(url, "/status") == 0) {
        ...
        snprintf(response, sizeof(response),
            "{\"status\":\"running\",\"type\":\"http\",\"url\":\"%s\","
            "\"output\":\"%s:%d\"}",
            g_ctx.http_url,
            g_ctx.output_addr, g_ctx.output_port);
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--url") == 0 && i + 1 < argc) {
            strncpy(g_ctx.http_url, argv[++i], sizeof(g_ctx.http_url) - 1);
            has_url = 1;
    ...
    fprintf(stderr, "HTTP Input Monitor Starting\n");
